...
import bundledConfig from "./default-config.cjs";
...
export class Config {
  ...
  validateDownstream(moduleID, stack = []) {
    ...
    for (const downstreamModule of downstream) {
      ...
      this.validateDownstream(downstreamModule, [...stack, downstreamModule]);
    }
  }
...
function getDefaultConfig() {
  if (!defaultConfigInstance) defaultConfigInstance = new Config(bundledConfig);
